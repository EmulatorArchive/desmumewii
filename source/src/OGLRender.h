...
#define DEFAULT_FIFO_SIZE (1024*1024) //May need to change to 256*1024
...
//Thanks profetlyn for lots of OGL conversion code (and the shader class) to get the Wii version of DeSmuME to compile:
...
class ProgramListNode
{
	...
	public:
		...
		// Contructor
		ProgramListNode(GLuint);
		
		// Methods
		GLuint getShader();
		GLuint getNextNode();
		void setNextNode(GLuint);
...
class Program
{
	...
	public:
		...
		bool shaderIsAttached(GLuint);
		void attachShader(GLuint);
		void detachShader(GLuint);
		int getNumberOfAttachedShaders();
